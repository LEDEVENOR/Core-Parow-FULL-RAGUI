...
function LeaveInstance()
	if Instance.host ~= nil then
		...
		--	ShowNotification(_U('left_instance'))
		end
...
AddEventHandler('instance:onPlayerEntered', function(instance, player)
	...
	--ShowNotification(_('entered_into', playerName))
end)
...
AddEventHandler('instance:onPlayerLeft', function(instance, player)
	...
	--ShowNotification(_('left_out', playerName))
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if InstanceInvite ~= nil then
			--ShowHelpNotification(_U('press_to_enter'))
		else
			Citizen.Wait(500)
		end
